...
# Signed 32-bit INT ceiling, the narrowest `last_page` column across our DBs.
MAX_LAST_PAGE = 2**31 - 1
...
async def update_rom_file_progress(
    ...
    Self-service per-user state, so it takes ROMS_USER_WRITE rather than the
    library-wide ROMS_WRITE, exactly as rom notes do.
    """
    ...
    if "last_page" in body:
        ...
        if last_page is not None:
            ...
            # Bounded so an out-of-range value is a 400 rather than an integer
            # column overflow at insert time.
            if not 0 <= last_page <= MAX_LAST_PAGE:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail=f"last_page must be between 0 and {MAX_LAST_PAGE}",
                )
